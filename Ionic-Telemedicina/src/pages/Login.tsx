...
const Login: React.FC = () => {
    ...
    const manejarIngreso = async () => {
        ...
        if (!r && !p) {
            try {
                // Llamamos a la API enviando RUT y contraseña
                const respuesta = await loginUsuarioApi({ rut: rut, contrasena: pass });
                
                // Guardamos el rol en el navegador para el resto de la app
                localStorage.setItem('usuarioRol', respuesta.usuario.rol);
                localStorage.setItem('usuarioNombre', respuesta.usuario.nombre);

                localStorage.setItem('clinicoEstado', respuesta.clinicoEstado);
                localStorage.setItem('clinicoSintomas', respuesta.clinicoSintomas);
                localStorage.setItem('clinicoDolor', respuesta.clinicoDolor);
                localStorage.setItem('clinicoCita', respuesta.clinicoCita);
                localStorage.setItem('clinicoMensaje', respuesta.clinicoMensaje);
                localStorage.setItem('usuarioId', respuesta.usuario.id);

                
                if (respuesta.usuario.rol === 'medico') {
                    history.replace('/medico-dashboard'); 
                } else {
                    history.replace('/home'); 
                }
            } catch (err: any) {
                alert(err.mensaje || 'Credenciales incorrectas o error de conexión.');
            }
        }
    }
